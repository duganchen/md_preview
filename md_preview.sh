...
# This CSS here might be worth looking at, as an alternative.
# http://benjam.info/panam/

TEMP=$(mktemp -d)

# Note that most of this is the CSS as an inline heredoc.

cat <<- EOF > "$TEMP/pandoc.css"
<style>
/*
...
pandoc -f gfm -t html -o "$TEMP/preview.html" --self-contained --include-in-header="$TEMP/pandoc.css" --quiet "$MD"
...
echo "$MD" | entr -s "pandoc -f gfm -t html -o $TEMP/preview.html --self-contained --include-in-header=$TEMP/pandoc.css --quiet $MD"
